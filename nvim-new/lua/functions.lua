...
function M.trigger_run_e2e_test(platform)
  ...
  return function()
    ...
    local rerun_choice = vim.fn.confirm('Automatically rerun failed jobs?', '1 Yes\n2 No', 2)
    if rerun_choice == 0 then
      vim.notify('Cancelled 󰜺 ', vim.log.levels.INFO)
      return
    end

    local smoke_only = smoke_only_choice == 1 and 'true' or 'false'
    local enable_automatic_rerun = rerun_choice == 1 and 'true' or 'false'
    ...
    local cmd = {
      ...
      'smoke_only=' .. smoke_only,
      '--field',
      'enable_automatic_rerun=' .. enable_automatic_rerun,
    }
...
function M.is_github_action_workflow()
